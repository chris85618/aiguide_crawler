...
public class LearningPoolTest {
    ...
    @Test
    public void addTask() {
        pool.addTask(new LearningTask(new ArrayList<>(), new int[]{1, 0, 1, 0}, "http://127.0.0.1:3000", "uui34yush2hd89heri".hashCode(), new TreeMap<>()));
        assertEquals(1, pool.getTaskSize());
    }

    @Test
    public void addResult() {
        pool.addResult(new LearningResult(new ArrayList<>() , false));
    ...
    public void takeTaskHasOneTask() {
        pool.addTask(new LearningTask(new ArrayList<>(), new int[]{1, 0, 1, 0}, "http://127.0.0.1:3000", "uui34yush2hd89heri".hashCode(), new TreeMap<String, String>()));
        LearningTask task = pool.takeTask();
        assertNotNull(task);
